		...
		this.opts = $.extend({
			...
			modal_file: String() +
				...
				'<form id="redactorUploadFileForm" method="post" action="" enctype="multipart/form-data">' +
					...
					'<div style="margin-top: 7px;">' +
						'<input type="file" id="redactor_file" name="file" accept=".pdf,.xls,.xlsx,.doc,.docx,.rtf,.zip,.rar"/>' +
					'</div>' +
				'</form><br>' +
				'</div>',
			...
			modal_image_edit: String() +
				...
				'<div id="redactor_modal_footer">' +
					...
					'<input type="button" name=2222"save" class="redactor_modal_btn" id="redactorSaveBtn" value="' + RLANG.save + '" />' +
				'</div>',
	...
	Redactor.prototype = {
		...
		fileUploadCallback: function(json)
		{
			...
			if (json !== false)
			{
				...
				if (json.filelink == 'error') {
					alert('Произошла ошибка при закачивании файла');
				} else {
